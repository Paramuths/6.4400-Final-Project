...
namespace GLOO {
    CollisionNode::CollisionNode(float integration_step): 
        ...
        sphere_mesh_ = PrimitiveFactory::CreateSphere(sphere_radius_, 50, 50);
        sphere_material_ = std::make_shared<Material>(glm::vec3(0.f, .5f, 1.f),
                                                    glm::vec3(0.f, .5f, 1.f),
                                                    glm::vec3(0.4f, 0.4f, 0.4f), 20.0f);
    ...
    void CollisionNode::Update(double delta_time) {
    ...
    int CollisionNode::IndexOf(int r, int c) {
        return r * num_cloth_col_ + c;
    }

    void CollisionNode::Init() {
        InitParticle();
        InitCollision();
        InitSystem();
    }

    void CollisionNode::InitParticle() {
        // Clear Stored Information
        particle_state_.positions = {};
        particle_state_.velocities = {};
        sphere_state_.positions = {glm::vec3(1.f, 0.f, -2.f)};
        sphere_state_.velocities = {glm::vec3(0.f, 0.f, .1f)};
        sphere_system_.SetFixedPoint(glm::vec3(1.f, 0.2f, 0.f));
    ...
    void CollisionNode::InitCollision() {
        // Sphere
    ...
    void CollisionNode::Advance(float start_time) {
        ...
        auto sphere_next_state = integrator_->Integrate(sphere_system_, sphere_state_, start_time, integration_step_);
        sphere_state_ = sphere_next_state;
    }

    void CollisionNode::SetPositions() {
        auto sphere_position = sphere_state_.positions[0];
        
        for (int i = 0; i < particle_state_.positions.size(); i++) {
            auto position = particle_state_.positions[i];
            if (glm::distance(position, sphere_position) < sphere_radius_) {
                particle_state_.positions[i] = sphere_position + (sphere_radius_ + 0.02f) * glm::normalize(position - sphere_position);
                particle_state_.velocities[i] = glm::vec3(0.f);
            }
        }
        ...
        sphere_pointer_->GetTransform().SetPosition(sphere_position);
    }

    void CollisionNode::SetIndices() {
    ...
    void CollisionNode::SetColors() {
        auto colors = make_unique<ColorArray>();
        for (int r = 0; r < num_cloth_row_; r++) {
            for (int c = 0; c < num_cloth_col_; c++) {
                auto color = ((r%2 == 0) && (c%2 == 0))? glm::vec4(1.f, 0.f, 1.f, 5.f): glm::vec4(1.f, 0.3f, 0.3f, 5.f);
