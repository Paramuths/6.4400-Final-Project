...
class ClothSystem : public ParticleSystemBase {
    ParticleState ComputeTimeDerivative(const ParticleState& state, float time) const override {
        ...
        auto wind_force = ((enable_wind_) && (float(rand())/RAND_MAX < 0.05))? max_wind_ * glm::vec3((float(rand())/RAND_MAX), (float(rand())/RAND_MAX), (float(rand())/RAND_MAX)): glm::vec3(0.f);

        for (int i = 0; i < num_particles; i++) {
            gradient_state.positions.push_back(state.velocities[i]);
            // Calculate acceleration
            auto gravity_force = particle_mass_[i] * gravity_;
            auto drag_force = -drag_constant_ * state.velocities[i];
            auto spring_force = spring_force_list[i];
            auto acceleration = (gravity_force + drag_force + spring_force + wind_force)/particle_mass_[i];
    ...
    void ToggleEnableWind() {
        enable_wind_ = !enable_wind_;
    }
    ...
    float drag_constant_ = 0.2f;
    float max_wind_ = 5.f;

    bool enable_wind_ = false;
};
}  // namespace GLOO
